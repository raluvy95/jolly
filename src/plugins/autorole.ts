...
export function autorole(client: BotWithCache<Bot>, member: Member, user: User) {
    ...
    try {
        if (user.toggles.bot) {
            return client.helpers.addRole(config.guildID, member.id, autorole.botRoleID!).catch(() => { })
        } else if (!member.toggles.pending) {
            return client.helpers.addRole(config.guildID, member.id, autorole.userRoleID!).catch(() => { })
        }
    } catch { return; }
}
